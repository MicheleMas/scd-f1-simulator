...
Le tipologie di invio analizzate sono principalmente due, pull e push. Nel primo caso le informazioni devono essere recuperate esplicitamente, richiedendole all’intermediario, nel secondo (tramite il modello publish subscribe) le entità si registrano per ricevere una determinata tipologia di dati, e da quel momento in poi ogni volta che un'informazione del tipo scelto viene generata, essa viene spedita a chi ne ha fatto richiesta. %TODO ho aggiunto le ultime due parole, mi sembrava giusto dirlo
...
Entrambe sono utili in determinate circostanze e quindi è stato deciso di adottarle ambedue.\\
Per questo motivo le informazioni dell’intermediario sono state divise in due categorie, quelle che sono necessarie a tutti e quelle che vengono distribuite su richiesta. Per le prime è stato disposto un sistema di push basato sul modello publish subscribe che permette alle entità interessate di ricevere i dati appena disponibili; per le seconde viene utilizzato un sistema di pull, creando una risorsa che contiene queste informazioni, e lasciando l’onere di recuperarle all’entità interessata.
